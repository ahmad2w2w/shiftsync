...
function getMaxHeight() {
  return Math.min(520, window.innerHeight - PAD * 2)
}

function fits(left: number, top: number, maxH: number, vw: number, vh: number) {
  return (
    left >= PAD &&
    top >= PAD &&
...
function computePosition(rect: DOMRect, point?: PopoverPoint) {
  ...
  const candidates = [
    ...
    { top: rect.top - maxH - GAP, left: rect.left },
  ...
  if (point) {
    candidates.unshift(
      ...
      { top: point.y - maxH - GAP, left: point.x - POPOVER_W / 2 },
    )
  }

  for (const c of candidates) {
    const left = Math.max(PAD, Math.min(c.left, vw - POPOVER_W - PAD))
    const top = Math.max(PAD, Math.min(c.top, vh - maxH - PAD))
    if (fits(left, top, maxH, vw, vh)) return { top, left, maxH }
  }

  return {
    top: Math.max(PAD, Math.min(rect.bottom + GAP, vh - maxH - PAD)),
